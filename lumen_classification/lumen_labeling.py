r''' CODE TO VISUALISE EACH MASK AND ITS GT. USING THE INFORMATION IS POSSIBLE TO COMPLETE THE EXCEL FILE
to verify the labeling use excel_extractor.py
Author Sonia 
'''
...
def lumen_detecion_thresholding(image,lume_size_up_th,lume_size_down_th,area):
    ...
    # POST PROCESSING
    ## find contourns and fill the inside
...
if __name__=='__main__':
    ...
    # IMAGES LOAD
        ...
        # labeled_image = np.array(Image.open(
        #             os.path.join(path,label_folder,patch_name_lab+'.jpg')
        #     ))
        #------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    # LUMEN DETECTION with threshold
        ...
        print(lume_num_label_new)
        for i in range(lume_num_label_new+1):
            pro=image_rgb.copy()
            pro_label=lume_labels_new.copy()
            if i==0:
                continue
